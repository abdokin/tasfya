          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center justify-between gap-2 mb-1">
              <span className="text-xs text-gray-500">
                {formatDate(fatwa.published_date)}
              </span>
            ...
            <Link href={`/fatwas/${fatwa.id}`} className="block">
              <h3 className="text-base font-medium mb-2 hover:text-primary truncate break-words">
                {fatwa.title}
              </h3>
            </Link>
          </div>
    ...
    >
      <CardContent className="p-0">
        <div className="p-6">
          ...
          <Link href={`/fatwas/${fatwa.id}`} className="block">
            <h3 className="text-lg font-semibold mb-2 hover:text-primary-700 hover:underline transition-colors break-words">
              {fatwa.title}
            </h3>
          </Link>

          <div className="text-sm text-gray-600 mb-4 max-w-full overflow-hidden">
            {fatwa.question && (
